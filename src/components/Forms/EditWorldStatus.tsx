...
const EditWorldStatus = ({status, setIdEditWorldStatus} : {status: String, setIdEditWorldStatus:Function}) => {
  ...
  return (
    <div className='h-[100dvh] w-[100dvw] fixed top-0 left-0 z-999 bg-[#000000aa] flex justify-center items-center'>
        {!isUpdated ? 
        <div className='bg-white rounded-2xl w-[35%] p-7'>
      ...
      <div className='mb-6'>
        ...
        {isDropdownOpen && <div className='flex flex-col w-full gap-2 mt-2'>
            {dropdownOptions.map(option => {
                return <div className='px-5 py-1 font-semibold text-sm text-black border-top border-bottom left-0 rounded-lg hover:bg-[#cbd5e168] cursor-pointer' onClick={() => handleNewSelection(option)}>{option}</div>
            })}
        </div>}
        </div>
      </div>
      ...
      <div className='bg-white rounded-2xl w-[35%] p-7 flex flex-col items-center justify-center'>
            <h3 className='text-title-sm text-center font-bold mb-5 text-black dark:text-white'>World Status Updated</h3>
            <p className='text-center text-black'>An email has been sent to owneremail@gmail.com to notify about the world status update.</p>
